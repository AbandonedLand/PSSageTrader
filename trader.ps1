...
class XCHAmount : Amount{
    [decimal]$amount
    [UInt128]$mojo

    XCHAmount([decimal]$amount) : base ([decimal]$amount) {
        $this.amount = [Math]::round($amount, 12)
        $this.mojo = [UInt128]([Math]::Round($this.amount * 1000000000000, 0))
    }
}

class CATAmount : Amount{
    [decimal]$amount
    [UInt128]$mojo

    CATAmount([decimal]$amount) : base ([decimal]$amount) {
        $this.amount = [Math]::round($amount  , 3)
        $this.mojo = [UInt128]([Math]::Round($this.amount * 1000, 0))
    }
...
class GridBot{
    ...
    BuildInitialOffers($side){
        ...
        foreach($grid in $this.grid){
            ...
            $response = $this.MakeOfferFromGrid($this.grid.IndexOf($grid), $side,$true, $true) | Out-Null
    ...
    SubmitAllOffers(){
        ...
        foreach($offer in $this.active_offers){
            $offer_data = Get-SageOffer -offer_id $offer.offer_id
    ...
    CheckOffers(){
        ...
        foreach($offer in $this.active_offers){
            $details = Get-SageOffer -offer_id $offer.offer_id
            if($details.status -eq "completed"){
                $this.completed_offers += $offer
                if($offer.side -eq "bid"){
                    $response = $this.MakeOfferFromGrid($offer.index, "ask",$false,$false) 
                    $this.active_offers += $response                    
                    $this.active_offers = $this.active_offers | Where-Object { $_.offer_id -ne $offer.offer_id }
                    $this.submitOffer($response.offer_id)
                }
                if($offer.side -eq "ask"){
                    $response = $this.MakeOfferFromGrid($offer.index, "bid",$false,$false) 
                    $this.active_offers += $response
                    $this.active_offers = $this.active_offers | Where-Object { $_.offer_id -ne $offer.offer_id }
                    $this.submitOffer($response.offer_id)
                }            
...
#$bot = [GridBot]::new()
#$bot.token_x_id = "xch"
#$bot.token_y_id = "wUSDC.b"
#$bot.token_x = $assets.($bot.token_x_id)
#$bot.token_y = $assets.($bot.token_y_id)
#$bot.token_x_amount = [XCHAmount]::new(50)
#$bot.token_y_amount = [CATAmount]::new(500)
#$bot.min_price = 10.90
#$bot.max_price = 9.50
#$bot.steps = 50
#$bot.percentage = 0.0035
#$bot.BuildGrid()
#$bot.grid | ft

$usd = [GridBot]::Load(".\bots\0e69628a-8207-430d-9d05-ecc5a6b0180f.json")
$xch = [GridBot]::Load(".\bots\7d7be782-2a34-4b61-96c4-2ca033ddc1d6.json")
...
function Start-Bots(){
    ...
    while($true){
        ...
        Write-Host "Checking offers..."
        $usd.CheckOffers()
        $usd.grid[($usd.completed_offers.index)].x_fee | ForEach-Object {$x_fee += $_}
        Write-Host "XCH Fee Collected : $($x_fee/1000000000000)"
        Write-Host "Outstanding USD Offered:"

        $xch.CheckOffers()
        $xch.grid[($xch.completed_offers.index)].y_fee | ForEach-Object {$y_fee += $_}
        Write-Host "USD Fee Collected : $($y_fee/1000)"
